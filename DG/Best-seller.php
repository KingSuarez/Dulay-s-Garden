...
<head>
    ...
    <title>Dulay's Best-Seller</title>
</head>
...
<ul>
        ...
        <li class="active"><a href="Best-seller.php">BEST-SELLERS</a>
        </li>
...
<h3 style="font-family: Tangerine, cursive; color: black; font-size: 40px; text-align:center;font-weight: 900;">Best Seller Product</h3>         
...
<div class="CategoriesSection_con">
  <?php
// Search Filter
$sql = "SELECT p.*, SUM(o.quantity) AS total_sold 
        FROM `products` p
        JOIN `order_list` o ON p.id = o.p_id";

if (!empty($search_query)) {
    $sql .= " AND (p.pname LIKE :search OR p.detail LIKE :search)";
}

$sql .= " GROUP BY p.id 
          HAVING total_sold >= 15 
...
?>
        <div class="SpecificProductCard" data-stock="<?= htmlspecialchars($fetch_products['stock']); ?>">
            <form action="" method="POST">
                <img src="prod/<?= htmlspecialchars($fetch_products['image']); ?>" alt="Product Image" name="image" class="img" style="width:100%; object-fit:cover;">
                <h1 style="font-size: 24px; font-family:poppins; font-weight:bolder;" class="pname" name="pname"><?= htmlspecialchars($fetch_products['pname']); ?></h1>
                ...
                <div style="margin-right: 20px;">
                    ...
                    <p><button class="open-modal-btn" type="button" data-product-id="<?= htmlspecialchars($fetch_products['id']); ?>">View Product</button></p>
                </div>
            </form>
        </div>
<?php
    ...
    echo '<p class="empty">No best-sellers found it is new website!</p>';
}
?>
